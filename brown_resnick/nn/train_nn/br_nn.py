...
for i in range(0, parts_number):
    ...
    current_images_name = (image_name + "/" + version + "/data/train/train_images_" + image_name
    ...
    current_parameters_name = (image_name + "/" + version + "/data/train/train_parameters_" + 
    ...
    current_classes_name = (image_name + "/" + version + "/data/train/train_classes_" + image_name
...
validation_images_name = (image_name + "/" + version + "/data/validation/validation_images_"
...
validation_parameters_name = (image_name + "/" + version + 
...
validation_classes_name = (image_name + "/" + version + 
                              "/data/validation/validation_classes_" + image_name + "_" 
                              + str(validation_total_number_of_parameters) + "_reps_" + 
                              str(validation_total_number_of_replicates) + ".npy")
y_val = np.load(validation_classes_name)

#Detect the cuda device on machine. In this case, there is only one gpu.
...
fig_name = (image_name + "/" + version + "/loss/br_" + image_name + "_" + version + "_loss_plot.png")
...
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper left')
fig_name = (image_name + "/" + version + "/accuracy/br_" + image_name + "_" + version + "_accuracy_plot.png")
plt.savefig(fig_name)
plt.clf()

#Save model architecture and model weights.
json_file = (image_name + "/" + version + "/model/br_" + image_name + "_" + version + "_nn.json")
model_json = final_model.to_json()
with open(json_file, "w") as json_file:
    json_file.write(model_json)

weights_file = (image_name + "/" + version + "/model/br_" + image_name + "_" + version + "_nn_weights.h5")
