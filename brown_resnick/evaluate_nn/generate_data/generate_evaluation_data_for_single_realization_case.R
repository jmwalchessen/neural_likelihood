...
#data consists of spatial fields and the corresponding parameters which generated the spatial fields. The evaluation data is n realizations of GP per m parameters 
#where the m parameters come from a grid over the parameter space.
...
data_x_test <- array(NA, dim = c(number_of_parameters, number_of_reps, n, n))
image_name <- paste(paste(as.character(n), "by", sep = "_"), as.character(n), sep = "_")
data_file_name <- paste(paste(paste(paste(paste(paste(local_folder, "evaluate_nn/generate_data/data", image_name, sep = "/"), "single/reps/200", sep = "/"), 
                                            "evaluation_images_9_by_9_density", sep = "/"), image_name, sep = "_"), as.character(number_of_reps), 
                                            sep = "_"), "npy", sep = ".")
parameters_file_name <- paste(paste(paste(paste(paste(paste(local_folder, "evaluate_nn/generate_data/data", image_name, sep = "/"), "single/reps/200", sep = "/"), 
...
{
  data_x_test[ipred,irep,,] <- rmaxstab(number_of_reps, coord, 
                                    cov.mod = "brown", 
