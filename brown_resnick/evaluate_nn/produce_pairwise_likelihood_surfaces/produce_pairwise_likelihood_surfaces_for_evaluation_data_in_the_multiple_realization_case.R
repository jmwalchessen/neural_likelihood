#This script is for producing the pairwise likelihood surfaces for the evaluation data in the multiple realization case. 
#Changing he distance constraint/delta given (see paper for explanation of the distance constraint/delta), will change
...
np <- import("numpy")
image_name <- paste(paste(as.character(image_size), "by", sep = "_"), as.character(image_size), sep = "_")
home_folder <- "/home/juliatest/Desktop/likelihood_free_inference/neural_likelihood/brown_resnick"
evaluation_images_file_name <- paste(paste(paste(paste(paste(paste(paste(paste(paste(paste(paste(home_folder, "evaluate_nn/generate_data/data", sep = "/"), 
                                                                                           image_name, sep = "/"), "multi", sep = "/"), as.character(number_of_multiple_realizations), sep = "/"),
                                                                         "reps/200", sep = "/"), 
                                                                   "evaluation_images_10_by_10_density", sep = "/"), image_name, sep = "_"), "multi", sep = "_"), 
                                                 as.character(number_of_multiple_realizations), sep = "_"),
                                           as.character(number_of_replications), sep = "_"), "npy", sep = ".")

evaluation_images <- np$load(evaluation_images_file_name)
flattened_evaluation_images <- array(NA, dim = c(number_of_parameters, number_of_replications, number_of_multiple_realizations, (image_size**2)))

for( ipred in 1:number_of_parameters)
{
  for(irep in 1:number_of_replications)
  {
    for(imulti in 1:number_of_multiple_realizations)
    {
      flattened_evaluation_images[ipred,irep,imulti,] <- flatten(evaluation_images[ipred,irep,imulti,,])
    }
  }
}
...
# estimator for the best parameter estimator for the given spatial fields using pairwise likelihood).
#parameters:
...
{
  fit <- fitmaxstab(flattened_evaluation_images_per_parameter[irep,,], coordinates_matrix, "brown",
                    weights = weights, 
                    method = "L-BFGS-B", 
                                                                ...
                                                                weights, number_of_replications, distance_constraint)
{
  ...
  for(irep in 1:2)
  {
    ...
    fit <- extract_pairwise_likelihood_function_via_fitmaxstab(coordinates_matrix = coordinates_matrix,
                                                               flattened_evaluation_images_per_parameter = flattened_evaluation_images[ipred,,,],
                                                               weights = weights, 
                                                               irep = irep)
  ...
  file_name <- paste(paste(paste(paste(paste(paste(paste(paste
              (paste(paste(paste(paste(paste(paste(paste(paste(paste(paste(local_folder, "data", sep = "/"), image_name, sep = "/"), "dist", sep = "/"), 
               as.character(distance_constraint), sep = "_"), sep = "/"), "multi", sep = "/"), as.character(number_of_multiple_realizations), sep = "/"),
               sep = "/"), "reps", sep = "/"), as.character(number_of_replications), sep = "/"), "pairwise_likelihood_surfaces_10_by_10_density", sep = "/"),
              image_name, sep = "_"), "image", sep = "_"), "multi", sep = "_"), as.character(number_of_multiple_realizations), sep = "_"), 
              as.character(number_of_replications), sep = "_"), as.character(ipred), sep = "_"), "npy", sep = ".")
...
clusterExport(cluster, c("flattened_evaluation_images", "coordinates_matrix", "weights", "grid", "local_folder", "image_name",
                         "fitmaxstab", "np", "smooth_grid", "range_grid", "generate_pairwise_likelihood_surfaces_per_parameter",
                         "number_of_replications", "extract_pairwise_likelihood_function_via_fitmaxstab", "distance_constraint",
                         "number_of_multiple_realizations"))
