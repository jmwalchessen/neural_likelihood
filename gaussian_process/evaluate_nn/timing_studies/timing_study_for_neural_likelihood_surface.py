...
def produce_unvectorized_neural_likelihood_surface(possible_length_scales, possible_variances, image, n):

    neural_likelihood_surface = np.zeros((len(possible_length_scales), len(possible_variances)))

    for i in range(len(possible_length_scales)):
        current_length_scale = possible_length_scales[i]
...
def produce_vectorized_neural_likelihood_surface(possible_length_scales, possible_variances, image, n):
    ...
    length_scales = (np.repeat(np.asarray(possible_length_scales), len(possible_variances), 
                        axis = 0)).reshape((number_of_parameter_pairs, 1))
    variances = []
    variances = (np.array(sum([(variances + possible_variances) for i 
                             in range(0, len(possible_length_scales))], []))).reshape((number_of_parameter_pairs,1))
    parameter_matrix = np.concatenate([length_scales, variances], axis = 1)
    psi_field = (multi_psi(image_matrix, parameter_matrix)).reshape((len(possible_variances), 
...
unvectorized_time_array_filename = (local_folder + "/evaluate_nn/timing_studies/data/" + image_name + 
                        "/unvectorized_neural_likelihood_surface_time_with_parallelization_on_laptop_"
                          + str(ipred) + ".npy")
